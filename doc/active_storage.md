---
title: Upgrading from Active Storage
---

This guide is aimed at helping Active Storage users migrate to Shrine.

1. Explanation of the key differences in design between Active Stroage and Shrine
2. Instructions how to migrate an existing app that uses Active Storage to Shrine
3. Extensive reference of Active Storage's interface with Shrine equivalents

## Overview

Shrine and Active Storage primarily differ in terms of configurability. Customisation is a first-class citizen of Shrine, via plugins or overriding. Active Storage makes customisation quite difficult. This is reflected in the API and Design.

### Uploader

In Active Storage, the attachment logic is configured directly inside Active Record models:
...
Provided you've run your Active Storage migrations, you will be relying on  a polymorphic join table to persist your attachments to a cloud storage service.
...
Whereas Shrine does not assume polymorphic joins, but defaults to persisting pointers to files on a column within your model itself, either as a text file or perhaps as a jsonb data type.

```ruby
  create_table "photos", force: :cascade do |t|    
    t.jsonb "file_data" # or t.string    
    # etc.
  end
```
